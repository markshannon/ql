...
predicate class_stores_to_attribute(ClassValue cls, SelfAttributeStore store, string name) {
    exists(CallableValue f | f = cls.declaredAttribute(_) and store.getScope() = f.getScope() and store.getName() = name) and
    /* Exclude classes used as metaclasses */
    not cls.getASuperType() = Value::named("type")
}

predicate should_override_eq(ClassValue cls, CallableValue base_eq) {
    not cls.declares("__eq__") and
    exists(ClassValue sup | sup = cls.getBase(_) and sup.declaredAttribute("__eq__") = base_eq |
        not exists(GenericEqMethod eq | eq.getScope() = sup.getScope()) and
        not exists(IdentityEqMethod eq | eq.getScope() = sup.getScope()) and
        not base_eq.getScope() instanceof IdentityEqMethod and
        not base_eq = Value::named("object").(ClassValue).declaredAttribute("__eq__")
    )
}
 ...
 */
predicate superclassEqExpectsAttribute(ClassValue cls, CallableValue base_eq, string attrname) {
    not cls.declares("__eq__") and
    exists(ClassValue sup | sup = cls.getBase(_) and sup.declaredAttribute("__eq__") = base_eq |
...
from ClassValue cls, SelfAttributeStore store, CallableValue base_eq
